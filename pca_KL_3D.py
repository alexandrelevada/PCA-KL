...
# Creates a KNN graph from the dataset (K = 25)
esparsa = sknn.kneighbors_graph(dados, 25, mode='connectivity', include_self=True)
A = esparsa.toarray()
...
terceiro_maior = w[:, ordem[-3]]

# Projection matrix
WpcaKL = np.array([maior_autovetor, segundo_maior, terceiro_maior])
...
terceiro_maior1 = w1[:, ordem[-3]]

# Projection matrix
Wpca = np.array([maior_autovetor1, segundo_maior1, terceiro_maior1])
...
for i in range(nclass+1):
    ...
    if (i == 0):
        ...
        cor = 'aqua'
    ax.scatter(novos_dados[0, indices], novos_dados[1, indices], novos_dados[2, indices], c=cor)
...
for i in range(nclass+1):
    ...
    if (i == 0):
        ...
        cor = 'aqua'
    ax.scatter(novos_dados_pca[0, indices], novos_dados_pca[1, indices], novos_dados_pca[2, indices], c=cor)
